			...
			if(!empty($_POST)){
				...
				if ( $validator->fails() ) {
					...
					if($file){
						...
						$file_originalname =substr($file_name,0,3); 
						if($file_originalname=='PPI')
						{
							for($i=5;$i<=$arrayCount;$i++){
								
								$DC =substr($allDataInSheet[$i]["A"],0,3);
								...
								if($allDataInSheet[$i]["A"] != "" and $DC !='D/C'){
									...
									$data[$i]['insurance_company'] 	= $allDataInSheet[$i]["B"];
									$data[$i]['AR_amount'] 			= ($allDataInSheet[$i]["E"]);
						...
						else
						{
							...
							{   		       
								$data['name'] 		       = trim($emapData["0"]);
								$data['company_id']        = $company_id;
								$data['insurance_company'] = trim($emapData["2"]);
							    $data['AR_amount'] 		   = trim($emapData["7"]);
								$data['date_to'] 		   = date('Y-m-d', strtotime('+1 month'));
