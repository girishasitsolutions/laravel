<?php
	class UserController extends BaseController { 
		...
		public function getDashBoard($company_id=null){	
			...
			$patients_chart_processing=$patients_chart_processing->where('status','like',trim('pro').'%')->where('patients.is_deleted', 0)->groupBy('date_from') ->get();
			
			$arr1 = $arr2 = array();
			foreach($patients_chart_processing as $val){
				$key = $val['value'];
				$key2   = array_search($val['date_from'], $patients_chart);
				$arr2[$key2] = $key;
			}
			$patients_chart_processing = array();
			foreach($patients_chart as $key=>$val){
			...
			$patients_chart_assistance=$patients_chart_assistance->where('status','like',trim('Ass').'%')->where('patients.is_deleted', 0)->groupBy('date_from') ->get();
			$arr1 = $arr2 = array();
			...
			$patients_chart_paid=$patients_chart_paid->where('status','like',trim('paid').'%')->whereIn('date_from',$patients_chart)->where('patients.is_deleted', 0)->groupBy('date_from')->get();
			$arr1 = $arr2 = array();
			...
			return View::make('backend.dashboard', compact('patients_chart_processing','patients_chart','patients_chart_assistance','patients_chart_paid'));
		...
		//export data
		public function export_data($company_id ="" )
		{
			$company_id = Input::get('user_id');
			DB::setFetchMode(PDO::FETCH_ASSOC);
			$company_export_data = DB::table('patients')->select('name','insurance_id','insurance_company','code','date_from','date_to','billed_amount','paid_amount','AR_amount','status','notes')->where('company_id', $company_id )->orderBy('id', 'ASC')->get();
			DB::setFetchMode(PDO::FETCH_CLASS);	
