									...
									if($Str_date_key != 'Y' and trim(strtoupper($allDataInSheet[$i]["H"])) == 'CLOSE'){
										...
										for($k=8;$k<=13;$k++){
											$column  = PHPExcel_Cell::stringFromColumnIndex($k);
											 if(isset($allDataInSheet[$i][$column]) and $allDataInSheet[$i][$column] !=""){
												//	$Str_date[$k] = $allDataInSheet[$i][$column];
												$data['name'] = trim($allDataInSheet[$i]["A"]);
												$data['insurance_company'] 	= trim($department);
												$data['AR_amount'] 			= ($allDataInSheet[$i][$column])?trim($allDataInSheet[$i][$column]):"";
												$data['company_id'] 		= $company_id;
												$date_toArr  	= (isset($allDataInSheet[$i][$column]) and $allDataInSheet[$i][$column] !="")?date('Y-m-d', strtotime($Str_date[$k])):"";
												$data['date_to'] = $data['date_from'] = $date_toArr;
												 
												 $check_Existing  				= DB::table('patients')->where('company_id', $company_id)->where('insurance_company', $data['insurance_company'])->where('name',$data['name'])->where('date_from',$date_toArr)->first();
