<?php
	class UserController extends BaseController { 
		...
		public function getDashBoard($company_id=null){	
			...
			$patients_chart_processing = DB::table('patients')->select('date_from', DB::raw('count(id) as value'))->where('company_id', $chart_company_id )->where('status','like',trim('pro').'%')->where('patients.is_deleted', 0)->groupBy('date_from') ->get();
			$arr1 = $arr2 = array();
			...
			$patients_chart_assistance = DB::table('patients')->select('date_from', DB::raw('count(id) as val_assistance'))->where('company_id', $chart_company_id )->where('status','like',trim('Ass').'%')->where('patients.is_deleted', 0)->groupBy('date_from') ->get();
			$arr1 = $arr2 = array();
			...
			//paid date and value
			$patients_chart_paid = DB::table('patients')->select('date_from', DB::raw('count(id) as val_paid'))->where('company_id', $chart_company_id )->where('status','like',trim('paid').'%')->whereIn('date_from',$patients_chart)->where('patients.is_deleted', 0)->groupBy('date_from')->get();
			$arr1 = $arr2 = array();
