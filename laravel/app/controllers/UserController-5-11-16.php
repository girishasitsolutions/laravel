	...
	class UserController extends BaseController { 
		...
		public function getDashBoard($company_id=null){	 
			...
			foreach($datesArr as $key=>$val){
				$value1 = $patients_chart_processing[] = (isset($arr2[$key]) and $arr2[$key])?$value1 + $arr2[$key]:$value1;
			}
			...
			$value1 = 0;
			foreach($datesArr as $key=>$val){
				$value1 = $patients_chart_assistance[] = (isset($arr2[$key]) and $arr2[$key])?$value1 + $arr2[$key]:$value1;
			}
			//paid date and value
			...
			$value1 = 0;
			foreach($datesArr as $key=>$val){
				$value1 = $patients_chart_paid[] = (isset($arr2[$key]) and $arr2[$key])?$value1 + $arr2[$key]:$value1;
			}
